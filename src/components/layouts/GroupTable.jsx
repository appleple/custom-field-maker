...
export function GroupTable(props) {
  ...
  return (
    <section>
      ...
      {fieldgroup.name && (
        <table
          className={classnames('js-fieldgroup-sortable', {
            'adminTable acms-admin-table-admin-edit': preview.acmscss,
          })}
          ref={tableRef}
        >
          <thead className={classnames({ 'acms-admin-hide-sp': preview.acmscss })}>
            <tr>
              <th
                ...
                <>
                  {fieldgroup &&
                    fieldgroup.items.map((item) => (
                      <th className={classnames({ 'acms-admin-table-left': preview.acmscss })}>
                        {item.title}
                        {item.tooltip && (
          ...
          <tbody>
            {preview.editMode === 'preview' ? null : `<!-- BEGIN ${fieldgroup.name}:loop -->`}
            <tr className="sortable-item">
              <td className="item-handle acms-admin-table-nowrap">
                {preview.acmscss && <i className="acms-admin-icon-sort" />}
              </td>
                      ...
                      return (
                        <tr key={index} className="acms-admin-form-item">
                          ...
                          <td className="acms-admin-form-item-input">
                            <TextInput item={item} preview={preview} />
                          </td>
                        </tr>
                      );
                    }
          ...
          <tfoot>
            <tr>
              <td colSpan={preview.direction === 'horizontal' ? groupLength + 2 : 3}>
                <input
                  type="button"
                  className={classnames('item-insert', { 'acms-admin-btn-admin': preview.acmscss })}
                  value="追加"
                />
              </td>
            </tr>
