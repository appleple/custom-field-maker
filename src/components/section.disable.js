    ...
    return (
      <Fragment>
        {(item.type === 'checkbox' || item.type === 'radio') ? (
          <legend>{this.getLabel(item)}</legend>
        ) : (
        ...
        {jsValidator && '<!-- <form action="" method="post" class="js-validator" enctype="multipart/form-data"> -->'}
          {customfield && customfield.map((item, index) => {
            if (item.type === 'text') {
              ...
              return (
              <p key={index}>
                {this.renderLabel(item, acmscss, jsValidator)}
                <span>
                  <input type={type} name={item.name} value={`{${item.name}}`} className={classnames({ 'acms-admin-form-width-full': acmscss })} {...(jsValidator ? { 'data-validator': item.name } : {})} />
