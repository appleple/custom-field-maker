        ...
        <table className={classnames({ 'adminTable acms-admin-table-admin-edit': acmscss })}>
          {direction === 'horizontal' && (
            <thead className={classnames({ 'acms-admin-hide-sp': acmscss })}>
              <tr>
                {groupitems.map((item) => (
                  <th className={classnames({ 'acms-admin-table-left': acmscss })}>{item.title}</th>
                ))}
              </tr>
            </thead>
          )}
          <tbody>
            ...
            <tr>
              ...
              >
                {groupitems.map((item) => {
                  if (item.type === 'text') {
                    return this.wrapTable(<td>{`{${item.name}}`}</td>, item.title);
                  } else if (item.type === 'textarea') {
                    return this.wrapTable(<td>{`{${item.name}}[escape|nl2br]`}</td>, item.title);
                    ...
                    return this.wrapTable(<td>{`{${item.name}@html}[raw]`}</td>, item.name);
