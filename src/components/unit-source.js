        ...
        {customunit.map(item => {
          if (item.type === 'text') {
            ...
            return (<tr>
              ...
              {item.alt && <Fragment>代替テキスト:<input type="text" name={`${item.name}{id}@alt`} value="{{name}@alt}" size="40" /></Fragment>}
              <input type="hidden" name="unit{id}[]" value={item.name} />
            ...
            return (<tr>
              ...
              <td>
                ...
                {item.fileName && <input type="hidden" name={`${item.name}{id}@filename`} value={item.fileName} />}
                {this.renderValidator(item, acmscss)}
              </td>
            </tr>);
          }
