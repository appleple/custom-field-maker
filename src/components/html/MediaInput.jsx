...
export const MediaInput = (props) => {
  ...
  return (
    <td className="js-media-field">
      {!useDropArea &&
        <>
          ...
          <div className="acms-admin-margin-top-mini">
            <button type="button" className={classnames('js-insert', { 'acms-admin-btn': maker.acmscss })} data-type={mediaType ? mediaType : 'all'}>メディアを選択</button>
            <button type="button" className={classnames('js-insert', { 'acms-admin-btn': maker.acmscss })} data-type={mediaType ? mediaType : 'all'} data-mode="upload">アップロード</button>
      ...
      {useDropArea &&
        <>
          <div className="js-droparea" data-thumbnail={`{${name}@thumbnail}`} data-type={mediaType ? mediaType : 'all'} data-thumbnail-type={`{${name}@type}`} data-width={`${dropAreaWidth}px`} data-height={`${dropAreaHeight}px`} />
          <p className="js-text acms-admin-text-danger" style={{ display: 'none' }}>許可されていないファイルのため挿入できません。</p>
          <div className="acms-admin-margin-top-mini">
            <button type="button" className={classnames('js-insert', { 'acms-admin-btn': maker.acmscss })} data-type={mediaType ? mediaType : 'all'}>メディアを選択</button>
          </div>
